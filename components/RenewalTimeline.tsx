...
const RenewalTimeline = ({ entries, onViewAll }: RenewalTimelineProps) => {
    ...
    const totalDue = entries.reduce((sum, { subscription }) => sum + personalPrice(subscription), 0);
    const dueSummary = currencies.size === 1
        ...
        <View className="mb-5 rounded-3xl border border-border bg-card p-4">
            <View className="mb-4 flex-row items-center justify-between gap-3">
                <View>
                    ...
                    <Text className="mt-0.5 text-sm font-sans-medium text-muted-foreground">What&apos;s due in the next 30 days</Text>
                </View>
            ...
            <View className="gap-2">
                {visibleEntries.map((entry) => (
                    <View key={entry.subscription.id} className="flex-row items-center gap-3 rounded-2xl bg-background p-3">
                        <SubscriptionAvatar name={entry.subscription.name} iconKey={entry.subscription.iconKey} className="size-11 rounded-xl" />
                        <View className="min-w-0 flex-1">
                            <Text className="text-sm font-sans-bold text-primary" numberOfLines={1}>{entry.subscription.name}</Text>
                            <Text className="mt-0.5 text-xs font-sans-medium text-muted-foreground">
                                {formatRenewalDate(entry)} · {entry.daysUntil === 0 ? 'due today' : `in ${entry.daysUntil} days`}
                            </Text>
                        </View>
            ...
            <Pressable className="mt-4 self-start" onPress={onViewAll} accessibilityRole="button" accessibilityLabel="View all subscriptions">
                <Text className="text-sm font-sans-bold text-accent">
                    {entries.length > visibleEntries.length ? `View ${entries.length - visibleEntries.length} more renewals` : 'View all subscriptions'}
                </Text>
            </Pressable>
        </View>
    );
