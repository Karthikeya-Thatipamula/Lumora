...
export const update = mutation({
  ...
  handler: async (ctx, patch) => {
    ...
    if (patch.reminderDaysBefore !== undefined && !REMINDER_DAY_OPTIONS.includes(patch.reminderDaysBefore)) {
      throw new Error(`Reminder days must be one of: ${REMINDER_DAY_OPTIONS.join(", ")}`);
    }

    if (patch.themePreference !== undefined && !["system", "light", "dark"].includes(patch.themePreference)) {
