...
export const update = mutation({
  ...
  handler: async (ctx, { id, ...patch }) => {
    ...
    await ctx.db.patch("subscriptions", id,{
      ...
      ...(patch.paymentMethod !== undefined ? { paymentMethod: normalizeText(patch.paymentMethod) } : {}),
...
export const resetUsage = mutation({
  ...
  handler: async (ctx, { id }) => {
    ...
    await ctx.db.patch("subscriptions", id,{ usageCount: 0, usageSince: new Date().toISOString() });
  },
});
...
export const setStatus = mutation({
  ...
  handler: async (ctx, { id, status }) => {
    ...
    await ctx.db.patch("subscriptions", id,{ status, statusChangedAt: new Date().toISOString() });
  },
});
