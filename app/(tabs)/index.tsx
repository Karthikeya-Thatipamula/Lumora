...
export default function App() {
    ...
    const discoveryPrompts = useMemo(
        () =>
            getDiscoveryPrompts(subscriptions, 6)
                .filter((prompt) => !dismissedGroups.includes(prompt.group.id))
                .slice(0, 2),
                        ...
                        <View className="home-balance-card">
                            ...
                            <View className="home-balance-row">
                                <AnimatedNumber
                                    ...
                                    value={
                                        nextRenewal ? personalPrice(nextRenewal.subscription) : 0
                                    }
                                    format={(amount) =>
                                        formatCurrency(
                                            amount,
                                            nextRenewal?.subscription.currency ?? currency,
                                        )
                                    }
                                />
                                <Text className="home-balance-date">
                                    {nextRenewal
                                        ? dayjs(nextRenewal.date).format('MM/DD')
                                        : '--/--'}
                                </Text>
                            </View>
                        </View>
